...
def count_words_functional(text):
    ...
    def count_reducer(counts, word):
        ...
        counts[word] += 1 if word in counts else 1
        return counts
    
    # TODO: Create functional pipeline:
    # text → words → counts
    words = text_to_words(text)
    counts = reduce(count_reducer, words, defaultdict(int))
    return counts
...
def count_words_functional_v2(text):
    ...
    words = [word.lower().replace('.', '').replace(',', '') for word in text.split()]
...
print("Functional result:", dict(result))
